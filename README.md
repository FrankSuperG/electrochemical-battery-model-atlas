...
<p align="center">
  ...
  <img alt="Projects with a reproduced core example: 18" src="https://img.shields.io/badge/reproduced-18%2F21-2b6cb0" />
</p>
...
| --- | --- |
| Projects indexed | 21 projects. |
| Core example reproduced | 18 projects with command-level evidence. |
| Partial core reproduction | 0 projects. |
| Unreproduced after targeted attempts | 3 projects with documented blockers. |
| Not tested | 0 projects. |
| Interface limitation | [PyBattMo](REPRODUCTIONS/pybattmo.md): Python P2D path tested; constructor API issue remains within the BattMo project. |
...
Reproduction records are maintained in [REPRODUCTIONS/](REPRODUCTIONS/). As of 2026-09-22, the Atlas has 18 projects with a reproduced core example, 3 unreproduced projects, 0 partial projects, 0 blocked projects, and 0 not-tested projects. Each project is counted once. A reproduced core path does not imply that all language interfaces or features pass. Dates of individual runs are recorded on the linked pages.
...
The Atlas has **21 projects**, each counted once in every summary. BattMo's MATLAB, Julia and Python entry points belong to **one BattMo project**; its [family page](MODELS/battmo.md) provides all three. A project is counted as reproduced when at least one documented core example passes, not when every feature or interface is verified. Licenses and test results remain specific to each implementation. [PyBattMo's independent audit](REPRODUCTIONS/pybattmo.md) records successful Python P2D calls and a constructor bug; BattMoApp remains untested. Neither adds another project. Extension lists in grouped rows are a union, not a promise of feature parity.
...
| `battmo`                      | BattMo              | DFN          | MATLAB + Julia + Python (interface) | MATLAB: GPL-3.0<br>Julia: MIT | copyleft / permissive | thermal, degradation | framework, continuum, research                   | [BattMo](MODELS/battmo.md)                                  |
