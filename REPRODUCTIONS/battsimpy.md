...
docker run --rm --platform linux/amd64 --user root \
  -v '<atlas-root>/.upstream/matthewpklein__battsimpy':/work \
...
- Actual output:
  ...
  - patched example config paths from author-local absolute paths to `/work/...` and disabled plotting with `PLOT_VOLT_ON=0`
